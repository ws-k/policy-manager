...
export function tiptapToBodyHtml(content: Record<string, unknown>): string {
  ...
  function nodePlainText(nodes: TiptapNode[] = []): string {
    return nodes
      .map((n) => (n.type === 'text' ? (n.text ?? '') : nodePlainText(n.content)))
      .join('')
  }

  function getHeadingId(node: TiptapNode): string {
    const text = nodePlainText(node.content)
  ...
  function nodeToHtml(node: TiptapNode): string {
    switch (node.type) {
      ...
      case 'heading': {
        ...
        return `<h${level} id="${id}"${style}>${inlineHtml(node.content)}</h${level}>`
      }
